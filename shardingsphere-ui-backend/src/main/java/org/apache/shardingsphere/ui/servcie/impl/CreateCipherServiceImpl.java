...
public class CreateCipherServiceImpl implements CreateCipherService {
    ...
    @Override
    public void createCipherField(String schemaName) {

        MetaDataPersistService activatedMetadataService = configCenterService.getActivatedMetadataService();
        Collection<RuleConfiguration> collection = activatedMetadataService.getSchemaRuleService().load(schemaName);// 规则信息
        if(collection.isEmpty()){
            return;
        }
        Map<String, DataSourceConfiguration> load = activatedMetadataService.getDataSourceService().load(schemaName);
        List<EncryptRuleConfiguration> encryptionList = collection.stream().map(e -> (EncryptRuleConfiguration) e).collect(Collectors.toList());
        // 加密规则
        ...
        List<EncryptTableRuleConfiguration> tables = Lists.newArrayList();
        for (EncryptRuleConfiguration e:encryptionList) {
            List<EncryptTableRuleConfiguration>  list = (List)e.getTables();
            tables.addAll(list);
        }
        ...
        if(!map.isEmpty()){
            ...
            for (Map.Entry<String, Collection<EncryptColumnRuleConfiguration>> encrypt : collect.entrySet()) {
                if(tableMap.containsKey(encrypt.getKey())){
                    ...
                    // 筛选两者共有的字段数据
                    List<ColumnInfoVO> sameField = columnInfoVOS.stream().filter(c -> plain.contains(c.getName())
                                    && !cipher.contains(c.getName())).collect(Collectors.toList());
    ...
    private void getMetaInfo(DataSourceConfiguration dataSourceConfiguration, String names, Map<QueryMetaDataRequest, List<TableInfoVO>> map){
        ...
        QueryMetaDataRequest request = getRequest(url, queryMetaDataRequest);
        request.setUsername((String)allProps.get("username"));
        request.setPassword((String)allProps.get("password"));
    ...
    private QueryMetaDataRequest getRequest(String url,QueryMetaDataRequest queryMetaDataRequest){
        ...
        queryMetaDataRequest.setDbType(dbType == "mysql" ? DbTypeEnum.MYSQL : DbTypeEnum.PGSQL);
