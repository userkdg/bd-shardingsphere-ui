...
public class CreateCipherServiceImpl implements CreateCipherService {
    ...
    @Override
    public List<Path> createIntegrationScriptPath(String schemaName, Boolean isCipher) {
        ...
        if(ruleConfiguration.isEmpty()){
            throw new ShardingSphereUIException(500, "schema不存在或规则为空!");
        }
        // 加载数据源
        ...
        if(first.isPresent()){
            ...
            throw new ShardingSphereUIException(500, "数据源不存在");
        }
        // 筛选表字段
        if(!dataSourceMetadata.isEmpty()){
            List<Path> allFilePath = getAllScriptPaths(schemaName, isCipher, encryptors, tables, dataSourceMetadata);
            log.info("{}", allFilePath);
            return allFilePath;
        }
        throw new ShardingSphereUIException(500, String.format("数据库不存在%s表，无法创建密文字段", names));
    }

    private List<Path> getAllScriptPaths(String schemaName, Boolean isCipher,
                                         List<Map<String, ShardingSphereAlgorithmConfiguration>> encryptors,
                                         List<EncryptTableRuleConfiguration> tables,
                                         Map<QueryMetaDataRequest, List<TableInfoVO>> dataSourceMetadata) {
        ...
        if(isCipher && !fieldList.isEmpty()){
            ...
            generateTempFile(allFilePath, schemaName, schemaName + "_1-1_创建密文列_", sqlList);
            // 创建密文列相关索引（明文列对应的索引）
            sqlList = mysqlFieldFactory.createCipherIndexes(dataSourceMetadata, fieldList, encryptors);
            generateTempFile(allFilePath, schemaName, schemaName + "_1-2_创建密文列索引_", sqlList);
            ...
            generateTempFile(allFilePath, schemaName, schemaName + "_2-1_明文列改为备份列_", sqlList);
            ...
            generateTempFile(allFilePath, schemaName, schemaName + "_2-2_密文列改为明文列_", renameCipherSqls);
            ...
            generateTempFile(allFilePath, schemaName, schemaName +"_3-1_创建备份列_", createPlainBakSqls);
            ...
            generateTempFile(allFilePath, schemaName, schemaName + "_3-2_备份列删除_", cleanPlainFieldDataSql);
            ...
            generateTempFile(allFilePath, schemaName, schemaName + "_3-3_备份列可空_", changePlainFieldNullable);
            ...
            generateTempFile(allFilePath, schemaName, schemaName + "_3-4_密文列非空_", changeCipherFieldNullable);

        }else {
            throw new ShardingSphereUIException(500, "字段已创建或不存在");
        }
    ...
    @SneakyThrows
    private Path generateTempFile(List<Path> allFilePath, String schemaName, String fileNamePrefix, List<String> sqlList) {
        String userDir = System.getProperty("user.dir");
        Path sqlPath = Files.createDirectories(Paths.get(userDir, "/kms-sql", schemaName));
